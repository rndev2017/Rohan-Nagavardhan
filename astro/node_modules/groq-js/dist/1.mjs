...
function hashField(field) {
  switch (field.type) {
    ...
    case "object":
      return `${field.type}:(${Object.entries(field.attributes).map(([key, value]) => `${key}:${hashField(value.value)}`).join(",")}):ref-${field.dereferencesTo}:${field.rest ? hashField(field.rest) : "no-rest"}`;
    case "union":
  ...
  return mapConcrete(
    ...
    (left) => (
      ...
      mapConcrete(rhs, scope, (right) => {
        switch ($trace('opCallNode "%s" %O', node.op, { left, right }), node.op) {
          case "==":
          case "!=":
            return {
              type: "boolean",
