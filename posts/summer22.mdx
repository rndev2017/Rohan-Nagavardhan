...
title: "Summer 2022"
date: "August 12, 2022"
description: "A short recap of my summer"
---
...
<img src={"/images/wander_post.png"} />
...
<video width="300" height="450" controls>
  <source 
    src="/videos/wander_demo.mp4" type="video/mp4" />
...
<Prism scrollAreaComponent="div" colorScheme="dark" language="json">
  {`{
...
<p style={{fontSize: '0.8rem'}}className="text-slate-400 italic">* Credit: Google Search documentation</p>
...
For this project, I spun up a container running my FastAPI REST API with a single endpoint
with the path `/parse`. The `/parse` endpoint did the following things upon
recieving a request:
...
<video height="750" controls>
  <source 
    src="/videos/logit_demo.mp4" type="video/mp4" />
Your browser does not support the video tag.
</video>
...
<p style={{marginBottom: '0.5rem'}} className="font-semibold">Metrics:</p>
...
<p style={{fontSize: '0.8rem'}}className="text-slate-400 italic">* Data provided by Netlify Analytics</p>
...
My first task involved moving a two applications deployed on one server and 
redeploying it on to another. One app was called "Server Restart Tool" which 
enabled engineers to remotely restart any server. It was built with Django and
deployed on a legacy web server tool called Internet Information Services. The
...
<img 
  src="/images/coverage-code.png"/>
<p style={{fontSize: '0.8rem'}}className="text-slate-400 italic">* Credit: Yuri Fedorov</p>
...
### ESVP
Below are some of the major contributions to my teams flagship product.
...
<img
  src={"/images/workflow_pre_reload.png"}/>
...
2. Has to be easily configrable
...
Shout out to Srikar Ananthoju for reading drafts of my first blog post 
